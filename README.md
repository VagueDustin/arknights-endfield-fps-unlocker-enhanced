...
**Everything installs cleanly but nothing changes in game.** Check that the folder shown at the top is the installation you actually launch. Endfield can be installed twice on one PC, through Epic and through the official Gryphline launcher, and a setup applied to the copy you do not start can never reach the game. Fate Engine preselects whichever install the game last ran from, warns when the managed folder is not that one, and shows **Different install running** in the header if you start the other copy. Recovery, Export diagnostics lists every installation found.
